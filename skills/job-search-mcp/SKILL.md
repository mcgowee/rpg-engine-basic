---
name: Job Search MCP
description: Search for jobs across LinkedIn, Indeed, Glassdoor, ZipRecruiter, and Google Jobs using the local JobSpy MCP server. Use when the user asks to find jobs, search job listings, or look for positions by role, location, or criteria.
slug: job-search-mcp
tags:
  - job-search
  - career
...
MCP server: `/home/mcgowee/services/jobspy-mcp/server.py`
Python: `/home/mcgowee/services/jobspy-mcp/venv/bin/python3`

**Always call the MCP server via exec:**
```bash
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"openclaw","version":"1.0"}}}
{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"scrape_jobs_tool","arguments":{"search_term":"ROLE","location":"LOCATION","site_name":["indeed","zip_recruiter"],"results_wanted":10}}}' \
  | /home/mcgowee/services/jobspy-mcp/venv/bin/python3 /home/mcgowee/services/jobspy-mcp/server.py 2>/dev/null \
  | tail -1 | python3 -c "import sys,json; r=json.load(sys.stdin); jobs=r['result']['jobs']; [print(f'{j[\"title\"]} @ {j[\"company\"]} ({j[\"location\"]}) — {j.get(\"job_url\",\"\")}') for j in jobs]"
```

Replace `ROLE` and `LOCATION` with actual values. For mcgowee: default location is `Denver, CO`, target roles are Data/ML/AI.

**Format results for WhatsApp** — no markdown tables, use bullet lists:
```
Found N jobs for [role] in [location]:
